...
let poweredBy = document.getElementById('powerd') ;
let nameB = ["@powerd by minamelad"] ; 
let quotes = 
[
    {
    ...
    let test = GivMeIndex(); 
    function autoWritten() { 
        quoteText.textContent = `"${quotes[test].quote.slice(0,charW)}"`; 
        author.textContent = `${quotes[test].author.slice(0,charW)}`; 
        poweredBy.textContent = nameB[0].slice(0,charW) ;
        charW++ ; 
        setTimeout(autoWritten,10) ; 
    }
    let mode = document.getElementById('change') ;
    // localStorage.clear();
    function CheckerBool() { 
    ...
    // white 
    // bool kan b true and now become false 
    function CurrentC() { 
        let z =  CheckerBool() ;
        console.log(z);  
        if (z === true) { 
            ...
            quoteText.style.color= "#212121"  ;
        }
        else { 
            mode.setAttribute('class','fa-solid fa-moon');
            document.body.style.backgroundColor = "#212121" ; 
            quoteText.style.color= "rgb(201 201 201)"  ;
        }
    }
    CurrentC() ; 
